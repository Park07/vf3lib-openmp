...
RESULT_DIR=~/vf3lib-openmp/results/sanity_openmp/run_1_8_$(date +%Y%m%d_%H%M%S)
mkdir -p "$RESULT_DIR"
cd "$RESULT_DIR"

VF3P=~/vf3lib-openmp/bin/vf3p
TEST_DIR=~/vf3lib-openmp/test
CSV_FILE="sanity_openmp_1_8.csv"
...
# Test with 1 and 8 threads only
...
echo ""
echo "OpenMP sanity check (1,8 threads) complete!"
cat "$CSV_FILE"
